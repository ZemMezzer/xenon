...
internal static class WorkspaceIndexBuilder
{
    ...
    private static string CreateDeclarationIdentity(Symbol symbol) => symbol switch
    {
        ...
        FunctionSymbol function => string.Join('|',
            "function",
            CreateDeclarationIdentity(function.ContainingSymbol!),
        ...
        IndexerSymbol indexer => string.Join('|',
            "indexer", CreateDeclarationIdentity(indexer.ContainingSymbol!), indexer.QualifiedName,
            Type(indexer.Type), Parameters(indexer.Parameters), indexer.IsReadonly),
        InterfaceIndexerSymbol indexer => string.Join('|',
            "interface-indexer", CreateDeclarationIdentity(indexer.ContainingSymbol!), indexer.QualifiedName,
            Type(indexer.Type), Parameters(indexer.Parameters), indexer.IsReadonly),
        FieldSymbol field => string.Join('|',
            "field", CreateDeclarationIdentity(field.ContainingSymbol!), field.QualifiedName,
            Type(field.Type), field.IsStatic, field.IsReadonly),
        PropertySymbol property => string.Join('|',
            "property", CreateDeclarationIdentity(property.ContainingSymbol!), property.QualifiedName,
            Type(property.Type)),
        InterfacePropertySymbol property => string.Join('|',
            "interface-property", CreateDeclarationIdentity(property.ContainingSymbol!), property.QualifiedName,
            Type(property.Type)),
        ConstantSymbol constant => string.Join('|',
            "constant", CreateDeclarationIdentity(constant.ContainingSymbol!), constant.QualifiedName,
            Type(constant.Type)),
        GenericParameterSymbol parameter => string.Join('|',
            "generic-parameter", CreateDeclarationIdentity(parameter.ContainingSymbol!), parameter.Ordinal),
        DeclaredTypeSymbol type => string.Join('|',
            "type", type.DeclarationKind, type.QualifiedName, type.GenericArity),

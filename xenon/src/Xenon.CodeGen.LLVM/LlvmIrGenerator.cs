...
public sealed class LlvmIrGenerator
{
        ...
        try
        {
            ...
            if (targetMachine is not null)
                _cAbi = new LlvmCAbi(
                    ...
                    MapType);
            var implementationFunctionBuilder = ImmutableArray.CreateBuilder<BoundFunction>();
            implementationFunctionBuilder.AddRange(compilation.GetStaticImplementationFunctions());
            foreach (LlvmNativeReference reference in wholeProgramReferences)
            {
                MarkWholeProgramSymbols(reference.Compilation);
                foreach (BoundFunction function in reference.Compilation.GetStaticImplementationFunctions()
            ...
            ImmutableArray<BoundFunction> implementationFunctions = implementationFunctionBuilder
    ...
    {
        LLVMTypeRef pointer = LLVMTypeRef.CreatePointer(_context.Int8Type, 0);
        LLVMTypeRef signature = LLVMTypeRef.CreateFunction(_context.VoidType, [pointer], false);
        foreach (FunctionSymbol function in functions.Select(item => item.Symbol)
                     .Where(item => item.IsCapturingLambda))
        {
            string name = MangleManagedName(_moduleIdentity, "closure_env_destructor", function.FullName);
            LLVMValueRef value = _module.AddFunction(name, signature);
            value.Linkage = LLVMLinkage.LLVMInternalLinkage;
            _closureEnvironmentDestructors.Add(function, new LlvmFunction(value, signature));
        }
    }

    private void EmitClosureEnvironmentDestructors()
    {
        foreach ((FunctionSymbol function, LlvmFunction destructor) in _closureEnvironmentDestructors)
        {
            using LLVMBuilderRef builder = _context.CreateBuilder();
            builder.PositionAtEnd(destructor.Value.AppendBasicBlock("entry"));
            LLVMValueRef environment = destructor.Value.GetParam(0);
            LLVMTypeRef environmentType = GetClosureEnvironmentType(function);
            CaptureVariableSymbol[] captures = function.LambdaCaptures.Reverse()
                .Where(capture => !capture.IsBorrow &&
                    TypeFacts.GetCompleteDestructor(capture.Type) is not null)
                .ToArray();
            for (int index = 0; index < captures.Length; index++)
            {
                ...
                if (_exceptionsEnabled)
                {
                    LLVMBasicBlockRef normal = destructor.Value.AppendBasicBlock(
                        $"{capture.Name}.destroy.continue");
                    LLVMBasicBlockRef unwind = destructor.Value.AppendBasicBlock(
                        $"{capture.Name}.destroy.unwind");
                    builder.BuildInvoke2(target.Type, target.Value, new[] { address }, normal, unwind,
                        string.Empty);
                    EmitClosureEnvironmentFailureCleanup(builder, destructor.Value, unwind,
                        environment, environmentType, captures[(index + 1)..]);
                    builder.PositionAtEnd(normal);
                }
                else
                {
                    builder.BuildCall2(target.Type, target.Value, new[] { address }, string.Empty);
                }
            }
            LlvmMemoryRuntime memory = GetOrDeclareMemoryRuntime();
            builder.BuildCall2(memory.FreeType, memory.Free, new[] { environment }, string.Empty);
            builder.BuildRetVoid();
        }
    }

    private unsafe void EmitClosureEnvironmentFailureCleanup(
        LLVMBuilderRef builder,
        LLVMValueRef helper,
        LLVMBasicBlockRef unwind,
        LLVMValueRef environment,
        LLVMTypeRef environmentType,
        IReadOnlyList<CaptureVariableSymbol> remaining)
    {
        LLVMBasicBlockRef continuation = helper.AppendBasicBlock("capture.destroy.exception.cleanup");
        builder.PositionAtEnd(unwind);
        if (IsWindowsTarget())
        {
            LLVMValueRef personality = GetOrDeclarePersonality(helper.GlobalParent, "__CxxFrameHandler3");
            helper.PersonalityFn = personality;
            ...
            LLVMBasicBlockRef handler = helper.AppendBasicBlock("capture.destroy.exception.catchpad");
            ...
            fixed (LLVMValueRef* argumentPointer = catchArguments)
            {
                ...
                LLVMApi.BuildCatchRet(builder, catchPad, continuation);
            }
        }
        else
        {
            LLVMValueRef personality = GetOrDeclarePersonality(helper.GlobalParent, "__gxx_personality_v0");
            helper.PersonalityFn = personality;
            LLVMTypeRef pointer = LLVMTypeRef.CreatePointer(_context.Int8Type, 0);
            ...
            LLVMValueRef landing = builder.BuildLandingPad(landingType, personality, 1,
                "capture.destroy.exception.landingpad");
            landing.AddClause(LLVMValueRef.CreateConstPointerNull(pointer));
            LLVMValueRef nativeException = builder.BuildExtractValue(landing, 0,
                "capture.destroy.native.exception");
            LLVMTypeRef beginType = LLVMTypeRef.CreateFunction(pointer, [pointer], false);
            LLVMValueRef begin = GetOrAddFunction(helper.GlobalParent, "__cxa_begin_catch", beginType);
            LLVMTypeRef endType = LLVMTypeRef.CreateFunction(_context.VoidType, [], false);
            LLVMValueRef end = GetOrAddFunction(helper.GlobalParent, "__cxa_end_catch", endType);
        ...
        LlvmFunction cleanup = GetOrDeclareExceptionRuntime().Cleanup;
        foreach (CaptureVariableSymbol capture in remaining)
        {
            FunctionSymbol captureDestructor = TypeFacts.GetCompleteDestructor(capture.Type)!;
            LLVMValueRef address = builder.BuildStructGEP2(environmentType, environment,
        ...
        LlvmMemoryRuntime memory = GetOrDeclareMemoryRuntime();
        builder.BuildCall2(memory.FreeType, memory.Free, new[] { environment }, string.Empty);
        LlvmFunction rethrow = GetOrDeclareExceptionRuntime().Rethrow;
        builder.BuildCall2(rethrow.Type, rethrow.Value, Array.Empty<LLVMValueRef>(), string.Empty);
        builder.BuildUnreachable();
    }
    ...
    private sealed unsafe class FunctionEmitter
    {
            ...
            bool exceptionsEnabled)
        {
            ...
            _closureEnvironment = function.IsCapturingLambda
            ...
            TypeSymbol type)
        {
            ...
            {
                if (current is AtomicTypeSymbol atomic)
                {
                    if (TypeFacts.GetCompleteDestructor(atomic.ElementType) is { } atomicDestructor)
                        units.Add((path, atomic, atomicDestructor));
                    return;
                }
                ...
                if (current is FunctionValueTypeSymbol { CompleteDestructor: { } functionValueDestructor })
                {
                    units.Add((path, current, functionValueDestructor));
                    return;
                }
        ...
        private void AllocateConstructorFieldInitializationFlags()
        {
            ...
            foreach (FieldSymbol field in owner.Fields)
            {
                if (TypeFacts.GetCompleteDestructor(field.Type) is null &&
                    !TypeFacts.ContainsAtomicStorage(field.Type))
                    continue;
        ...
        private void EmitVariableDeclaration(BoundVariableDeclarationStatement statement)
        {
            ...
            if (statement.Initializer is not null)
            {
                ...
                if (statement.Variable.Type is AtomicTypeSymbol atomic)
            ...
            BoundExpression source)
        {
            ...
            LLVMValueRef data = GetValueStorageAddress(
                ArrayData(arrayValue, cleanup.ArrayType), cleanup.ArrayType.ElementType);
            ...
            ImmutableArray<FieldSymbol> initializedPath = default)
        {
            ...
            foreach (ScalarCleanupEntry cleanup in cleanups)
            {
                ...
                _builder.PositionAtEnd(register);
                LLVMValueRef destructorAddress = ScalarCleanupValueAddress(address, cleanup);
                TypeSymbol cleanupValueType = cleanup.ValueType is AtomicTypeSymbol atomic
                    ? atomic.ElementType
                    : cleanup.ValueType;
                EmitCleanupRegistration(cleanup.Node, _scalarScopeHeads[variable], destructorAddress,
                    SizeConstant(1), SizeConstant(_getAbiSize(cleanupValueType)), cleanup.DestructorFunction);
                _builder.BuildStore(LLVMValueRef.CreateConstInt(_context.Int1Type, 1), cleanup.Initialized);
                _builder.BuildBr(end);
                _builder.PositionAtEnd(end);
                _builder.BuildStore(LLVMValueRef.CreateConstInt(_context.Int1Type, 1),
        ...
        private LLVMValueRef EmitExpressionCore(BoundExpression expression) => expression switch
        {
            ...
            BoundCompareExchangeExpression compareExchange => EmitCompareExchange(compareExchange),
            ...
            BoundFunctionValueExpression value => EmitFunctionValue(value),
            BoundFunctionValueCallExpression call => EmitFunctionValueCall(call),
            ...
            BoundStaticFieldExpression field => EmitLoad(field.Type, GetStaticFieldAddress(field.Field), field.Field.Name),
            ...
            BoundFunctionValueDestructionExpression destruction => EmitFunctionValueDestruction(destruction),
        ...
        {
            LLVMValueRef address = expression.Variable is CaptureVariableSymbol capture
                ? EmitCaptureAddress(capture)
                : GetAddress(expression.Variable);
            return EmitLoad(expression.Type, address, expression.Variable.Name);
        }
        ...
        private LLVMValueRef EmitCompareExchange(BoundCompareExchangeExpression expression)
        {
            ...
            if (TypeIdentity.AreSame(atomic.ElementType, BuiltinTypes.Bool))
            {
                expected = _builder.BuildZExt(expected, _mapType(atomic), "cmpxchg.expected.bool.storage");
                desired = _builder.BuildZExt(desired, _mapType(atomic), "cmpxchg.desired.bool.storage");
            }
            else if (atomic.ElementType is PrimitiveTypeSymbol { IsFloatingPoint: true })
            {
                return EmitFloatingCompareExchange(
                    address, atomic.ElementType, expected, desired);
            }

            LlvmCompareExchangeResult result = _atomics.CompareExchange(
                address,
                expected,
                desired,
                LLVMAtomicOrdering.LLVMAtomicOrderingSequentiallyConsistent,
        ...
        private LLVMValueRef EmitLockBackedCompareExchange(
            LLVMValueRef address,
            AtomicTypeSymbol atomic,
            LLVMValueRef expected,
            LLVMValueRef desired,
            ...
            LifecycleValueGuard desiredGuard)
        {
            ...
            LLVMValueRef valueAddress = AtomicValueAddress(address, atomic);
            LLVMValueRef current = _builder.BuildLoad2(
                _mapType(atomic.ElementType), valueAddress, "compare.exchange.current");
            LLVMValueRef succeeded = EmitValueEquality(current, expected, atomic.ElementType);
            _builder.BuildStore(
                _builder.BuildSelect(succeeded, desired, current, "compare.exchange.selected"),
                valueAddress);
            ReleaseAtomicLock(address, atomic);
        ...
        private void DestroyAtomicValue(LLVMValueRef value, TypeSymbol type, string name)
        {
            if (TypeFacts.GetCompleteDestructor(type) is not { } destructor) return;
            LLVMValueRef temporary = _builder.BuildAlloca(_mapType(type), $"{name}.destroy.tmp");
            _builder.BuildStore(value, temporary);
            EmitLifecycleCall(destructor, temporary, []);
        }
        ...
        private LLVMValueRef EmitExplicitDestruct(BoundExplicitDestructExpression expression)
        {
            ...
            if (expression.Destructor is not null)
            {
                LLVMValueRef address = EmitAddress(expression.Target);
                EmitLifecycleCall(expression.Destructor,
                    GetValueStorageAddress(address, expression.Target.Type), []);
            }
            return default;
        }
        ...
        private LLVMValueRef EmitFunctionValue(BoundFunctionValueExpression expression)
        {
            LLVMTypeRef valueType = _mapType(expression.FunctionValueType);
            LLVMTypeRef pointer = LLVMTypeRef.CreatePointer(_context.Int8Type, 0);
            LLVMValueRef control = LLVMValueRef.CreateConstPointerNull(pointer);
            if (!expression.Captures.IsEmpty)
            {
                var captures = new LLVMValueRef[expression.Captures.Length];
                var captureGuards = new LifecycleValueGuard?[expression.Captures.Length];
                for (int index = 0; index < expression.Captures.Length; index++)
                {
                    BoundFunctionValueCapture capture = expression.Captures[index];
                    LLVMValueRef capturedValue = EmitExpression(capture.Initializer);
                    captures[index] = capturedValue;
                    if (!capture.Variable.IsBorrow &&
                        TypeFacts.GetCompleteDestructor(capture.Variable.Type) is { } destructor)
                ...
                foreach (BoundFunctionValueCapture capture in expression.Captures)
                {
                    ...
                    offset += Math.Max(1UL, _getAbiSize(capture.Variable.StorageType));
                }
                LLVMValueRef environment = EmitAllocation(SizeConstant(Math.Max(1UL, offset)), "closure.environment");
                AllocationGuard environmentGuard = BeginAllocationGuard(environment);
                LLVMTypeRef environmentType = ClosureEnvironmentType(expression.InvokeFunction);
                for (int index = 0; index < captures.Length; index++)
                {
                ...
                control = EmitAllocation(SizeConstant(checked(pointerBytes * 3)), "closure.control");
                AllocationGuard controlGuard = BeginAllocationGuard(control);
                _builder.BuildStore(SizeConstant(1), FunctionControlField(control, 0, "closure.strong.address"));
                ...
                foreach (LifecycleValueGuard? guard in captureGuards)
                    if (guard is not null) CompleteLifecycleValueGuard(guard);
            }

            LLVMValueRef value = valueType.Poison;
            LLVMValueRef invoke = expression.InvokeFunction.IsLambda
                ? _functions[expression.InvokeFunction].Value
                : _getFunctionValueInvokeAddress(expression.InvokeFunction);
            value = _builder.BuildInsertValue(value, invoke, 0, "function.invoke.init");
            return _builder.BuildInsertValue(value, control, 1, "function.control.init");
        }
        ...
        private LLVMValueRef EmitReferenceDereference(BoundReferenceDereferenceExpression expression) =>
            EmitLoad(expression.ReferenceType.ElementType,
        ...
        private LLVMValueRef EmitIncrement(BoundUnaryExpression expression)
        {
            ...
            {
                bool floatingPoint = atomic.ElementType is PrimitiveTypeSymbol { IsFloatingPoint: true };
                LLVMValueRef atomicOne = floatingPoint
                    ? LLVMValueRef.CreateConstReal(_mapType(atomic.ElementType), 1.0)
                    : LLVMValueRef.CreateConstInt(_mapType(atomic.ElementType), 1, false);
                LLVMAtomicRMWBinOp operation = (expression.OperatorKind, floatingPoint) switch
                {
                    (SyntaxKind.PlusPlusToken, true) => LLVMAtomicRMWBinOp.LLVMAtomicRMWBinOpFAdd,
                    (SyntaxKind.MinusMinusToken, true) => LLVMAtomicRMWBinOp.LLVMAtomicRMWBinOpFSub,
                    (SyntaxKind.PlusPlusToken, false) => LLVMAtomicRMWBinOp.LLVMAtomicRMWBinOpAdd,
                    _ => LLVMAtomicRMWBinOp.LLVMAtomicRMWBinOpSub,
                };
                LLVMValueRef previous = _atomics.Fetch(
                    operation,
                    address,
                    atomicOne,
                    LLVMAtomicOrdering.LLVMAtomicOrderingSequentiallyConsistent,
                    "atomic.increment.previous");
                if (expression.IsPostfix) return previous;
                return EmitArithmetic(
                    expression.OperatorKind == SyntaxKind.PlusPlusToken ? SyntaxKind.PlusToken : SyntaxKind.MinusToken,
                    atomic.ElementType,
                    previous,
                    atomicOne);
            }
        ...
        private LLVMValueRef EmitComparison(
            ...
            LLVMValueRef right)
        {
            if (expression.OperatorKind is SyntaxKind.EqualsEqualsToken or SyntaxKind.BangEqualsToken)
            {
                LLVMValueRef equal = EmitValueEquality(left, right, expression.Left.Type);
                return expression.OperatorKind == SyntaxKind.EqualsEqualsToken
                    ? equal
        ...
        private LLVMValueRef EmitAssignment(BoundAssignmentExpression expression)
        {
            if (expression.Target.Type is AtomicTypeSymbol atomic)
                return EmitAtomicAssignment(expression, atomic);
                ...
                expression.MovedPlaceReinitialization != MovedPlaceReinitializationState.DefinitelyMoved)
            {
                ...
                {
                    LLVMValueRef initialized = LoadConstructorFieldInitializationFlag(expression);
                    LLVMBasicBlockRef replace = _llvmFunction.AppendBasicBlock("constructor.field.replace");
                    LLVMBasicBlockRef store = _llvmFunction.AppendBasicBlock("constructor.field.store");
                    _builder.BuildCondBr(initialized, replace, store);
                    _builder.PositionAtEnd(replace);
                    EmitReplacementDestruction(expression, address);
                    _builder.BuildBr(store);
                    _builder.PositionAtEnd(store);
                }
            ...
            AtomicTypeSymbol atomic)
        {
            ...
            const LLVMAtomicOrdering ordering =
                LLVMAtomicOrdering.LLVMAtomicOrderingSequentiallyConsistent;
            if (expression.OperatorKind == SyntaxKind.EqualsToken)
            {
                if (expression.RequiresRuntimeInitializationCheck)
                {
                    LLVMValueRef initialized = LoadConstructorFieldInitializationFlag(expression);
                    LLVMBasicBlockRef replace = _llvmFunction.AppendBasicBlock("constructor.atomic.replace");
                    LLVMBasicBlockRef initialize = _llvmFunction.AppendBasicBlock("constructor.atomic.initialize");
                    LLVMBasicBlockRef end = _llvmFunction.AppendBasicBlock("constructor.atomic.ready");
                    _builder.BuildCondBr(initialized, replace, initialize);
                    _builder.PositionAtEnd(replace);
                    EmitAtomicReplacement(value, address, atomic);
                    _builder.BuildBr(end);
                    _builder.PositionAtEnd(initialize);
                    EmitAtomicStore(value, address, atomic, initialize: true);
                    _builder.BuildBr(end);
                    _builder.PositionAtEnd(end);
                }
                else if (IsLockBackedAtomic(atomic) && !expression.IsInitialization)
                    EmitAtomicReplacement(value, address, atomic);
                else
                    EmitAtomicStore(value, address, atomic, initialize: expression.IsInitialization);
                MarkConstructorFieldInitialized(expression);
                if (TryGetScalarProjection(expression.Target, out VariableSymbol variable,
                        out ImmutableArray<FieldSymbol> initializedPath))
                    EmitScalarRegistration(variable, GetAddress(variable), initializedPath);
                return value;
            }

            LLVMAtomicRMWBinOp operation = expression.OperatorKind switch
            {
                SyntaxKind.PlusEqualsToken when atomic.ElementType is PrimitiveTypeSymbol { IsFloatingPoint: true } =>
                    LLVMAtomicRMWBinOp.LLVMAtomicRMWBinOpFAdd,
                SyntaxKind.MinusEqualsToken when atomic.ElementType is PrimitiveTypeSymbol { IsFloatingPoint: true } =>
                    LLVMAtomicRMWBinOp.LLVMAtomicRMWBinOpFSub,
                SyntaxKind.PlusEqualsToken => LLVMAtomicRMWBinOp.LLVMAtomicRMWBinOpAdd,
                SyntaxKind.MinusEqualsToken => LLVMAtomicRMWBinOp.LLVMAtomicRMWBinOpSub,
                SyntaxKind.AmpersandEqualsToken => LLVMAtomicRMWBinOp.LLVMAtomicRMWBinOpAnd,
                SyntaxKind.PipeEqualsToken => LLVMAtomicRMWBinOp.LLVMAtomicRMWBinOpOr,
                SyntaxKind.CaretEqualsToken => LLVMAtomicRMWBinOp.LLVMAtomicRMWBinOpXor,
                _ => throw new LlvmCodeGenerationException(
                    $"Atomic assignment operator '{expression.OperatorKind}' is not supported."),
            };
            LLVMValueRef previous = _atomics.Fetch(operation, address, value, ordering, "atomic.previous");
            return EmitArithmetic(
                GetBinaryOperatorForCompoundAssignment(expression.OperatorKind),
                atomic.ElementType,
                previous,
        ...
        private void EmitReplacementDestruction(BoundAssignmentExpression expression, LLVMValueRef address)
        {
            ...
            {
                bool trackedDestruction = EmitAssignmentDestinationDestruction(
                    destination, GetAddress(destination), destinationPath);
                if (!trackedDestruction &&
            ...
            else if (TypeFacts.GetCompleteDestructor(expression.Target.Type) is { } destructor)
            ...
            AtomicTypeSymbol atomic)
        {
            if (!IsLockBackedAtomic(atomic))
            {
                EmitAtomicStore(value, address, atomic);
                return;
            }
            AcquireAtomicLock(address, atomic, "atomic.replace");
            LLVMValueRef valueAddress = AtomicValueAddress(address, atomic);
            LLVMValueRef compositePrevious = _builder.BuildLoad2(
                _mapType(atomic.ElementType), valueAddress, "atomic.replace.previous");
            _builder.BuildStore(value, valueAddress);
            ReleaseAtomicLock(address, atomic);
            DestroyAtomicValue(compositePrevious, atomic.ElementType, "atomic.replace.previous");
        }

        private LLVMValueRef LoadConstructorFieldInitializationFlag(BoundAssignmentExpression expression)
        {
            if (expression.ConstructorField is null ||
                !_constructorFieldInitializationFlags.TryGetValue(expression.ConstructorField, out LLVMValueRef flag))
                throw new LlvmCodeGenerationException(
                    "conditional constructor field assignment has no initialization state");
            return _builder.BuildLoad2(_context.Int1Type, flag, "constructor.field.initialized");
        }

        private void MarkConstructorFieldInitialized(BoundAssignmentExpression expression)
        {
            if (expression.ConstructorField is not null &&
                _constructorFieldInitializationFlags.TryGetValue(expression.ConstructorField, out LLVMValueRef flag))
                _builder.BuildStore(LLVMValueRef.CreateConstInt(_context.Int1Type, 1), flag);
        }

        private LLVMValueRef EmitSwap(BoundSwapExpression expression)
        {
            LLVMValueRef leftAddress = EmitAddress(expression.Left);
            LLVMValueRef rightAddress = EmitAddress(expression.Right);
            if (expression.Left.Type is AtomicTypeSymbol leftAtomic)
            {
                LLVMValueRef replacement = _builder.BuildLoad2(
                    _mapType(leftAtomic.ElementType), rightAddress, "swap.replacement");
                LLVMValueRef previous = EmitAtomicExchange(replacement, leftAddress, leftAtomic);
                _builder.BuildStore(previous, rightAddress);
                return default;
            }
            if (expression.Right.Type is AtomicTypeSymbol rightAtomic)
            {
                LLVMValueRef replacement = _builder.BuildLoad2(
                    _mapType(rightAtomic.ElementType), leftAddress, "swap.replacement");
                LLVMValueRef previous = EmitAtomicExchange(replacement, rightAddress, rightAtomic);
                _builder.BuildStore(previous, leftAddress);
                return default;
            }

            LLVMValueRef left = _builder.BuildLoad2(_mapType(expression.Left.Type), leftAddress, "swap.left");
            LLVMValueRef right = _builder.BuildLoad2(_mapType(expression.Right.Type), rightAddress, "swap.right");
            _builder.BuildStore(right, leftAddress);
            _builder.BuildStore(left, rightAddress);
            return default;
        }
        ...
        private LLVMValueRef EmitMemberAccess(BoundMemberAccessExpression expression)
        {
            ...
            LLVMValueRef address = EmitAddress(expression);
            return EmitLoad(expression.Type, address, expression.Field.Name);
        }
        ...
        private LLVMValueRef EmitArrayCreation(BoundArrayCreationExpression expression)
        {
            ...
            StructTypeSymbol? structure = expression.ElementType switch
            {
        ...
        private LLVMValueRef EmitFunctionValueDestruction(BoundFunctionValueDestructionExpression expression)
        {
            LLVMValueRef storageAddress = _llvmFunction.GetParam(0);
            ...
            _builder.BuildCondBr(_builder.BuildICmp(LLVMIntPredicate.LLVMIntNE, control,
                LLVMValueRef.CreateConstPointerNull(control.TypeOf), "function.release.valid"), release, end);
            _builder.PositionAtEnd(release);
            LLVMValueRef previous = _atomics.FetchSub(
                FunctionControlField(control, 0, "function.release.count.address"),
                SizeConstant(1), LLVMAtomicOrdering.LLVMAtomicOrderingRelease, "function.release");
            _builder.BuildCondBr(_builder.BuildICmp(LLVMIntPredicate.LLVMIntEQ, previous, SizeConstant(1),
                "function.release.final"), destroy, end);
            _builder.PositionAtEnd(destroy);
            _atomics.AcquireFence();
            LLVMTypeRef pointer = LLVMTypeRef.CreatePointer(_context.Int8Type, 0);
            LLVMValueRef environment = _builder.BuildLoad2(pointer,
                FunctionControlField(control, 1, "function.release.environment.address"),
                "function.release.environment");
            LLVMValueRef destructor = _builder.BuildLoad2(pointer,
                FunctionControlField(control, 2, "function.release.destructor.address"),
                "function.release.destructor");
            AllocationGuard controlGuard = BeginAllocationGuard(control);
            LLVMTypeRef signature = LLVMTypeRef.CreateFunction(_context.VoidType, [pointer], false);
            EmitPotentiallyThrowingCall(signature, destructor, [environment], string.Empty);
            CompleteAllocationGuard(controlGuard);
            LlvmMemoryRuntime memory = _getMemoryRuntime();
            _builder.BuildCall2(memory.FreeType, memory.Free, new[] { control }, string.Empty);
            _builder.BuildBr(end);
            _builder.PositionAtEnd(end);
            return default;
        }
            ...
            bool initializeVTable = true)
        {
            ...
            LLVMValueRef result = EmitLifecycleTargetCall(
                llvmFunction.Type, llvmFunction.Value, values, function);
        ...
        private LLVMValueRef EmitLifecycleTargetCall(
            ...
            LLVMValueRef target,
            LLVMValueRef[] arguments,
            FunctionSymbol? directTarget = null)
        {
            if (directTarget is not null &&
                _functionEffects.TryGetValue(directTarget, out LlvmFunctionEffects effects) &&
                !effects.MayThrow)
            {
                return _builder.BuildCall2(functionType, target, arguments, string.Empty);
            }
        ...
        private LLVMValueRef EmitIndex(BoundIndexExpression expression)
        {
            ...
            return EmitLoad(expression.ElementType, address, "element");
        }
        ...
        {
            BoundVariableExpression { Variable: CaptureVariableSymbol capture } => EmitCaptureAddress(capture),
            BoundVariableExpression variable => GetAddress(variable.Variable),
            BoundStaticFieldExpression field => GetStaticFieldAddress(field.Field),
        ...
        private LLVMValueRef GetStaticFieldAddress(FieldSymbol field)
        {
        ...
        {
            LLVMValueRef value = EmitExpression(expression.Expression);
            if (expression.Expression.Type is AtomicTypeSymbol atomic &&
        ...
        private LLVMValueRef EmitMethodCall(BoundMethodCallExpression expression)
        {
            ...
            LLVMValueRef result = EmitPotentiallyThrowingCall(expression.Method, function, arguments, name);
        ...
        private LLVMValueRef EmitPropertySet(BoundPropertySetExpression expression)
        {
            ...
            if (setter.VTableSlot is not int slot)
            {
                ...
                EmitPotentiallyThrowingCall(setter, function, new[] { receiver, value }, string.Empty);
                return value;
            }
        ...
        private LLVMValueRef EmitIndexerSet(BoundIndexerSetExpression expression)
        {
            ...
            {
                LlvmFunction function = _functions[setter];
                EmitPotentiallyThrowingCall(setter, function, arguments, string.Empty);
                return value;
            }
        ...
        private LLVMValueRef EmitCall(BoundCallExpression expression)
        {
            ...
            return EmitPotentiallyThrowingCall(expression.Function, function, arguments, name);
        }
        ...
        private LLVMValueRef EmitFunctionValueCall(BoundFunctionValueCallExpression expression)
        {
            LLVMValueRef value = EmitExpression(expression.Target);
            ...
            if (_enableRuntimeChecks)
                EmitRuntimeCheck(_builder.BuildICmp(LLVMIntPredicate.LLVMIntNE, invoke,
                    LLVMValueRef.CreateConstPointerNull(invoke.TypeOf), "function.invoke.valid"));
            EmitFunctionControlRetain(control);
            FunctionSymbol valueDestructor = expression.FunctionValueType.CompleteDestructor ??
                throw new LlvmCodeGenerationException(
                    $"Function value type '{expression.FunctionValueType.ToDisplayString()}' has no destructor.");
            LifecycleValueGuard callGuard = BeginLifecycleValueGuard(
                value, expression.FunctionValueType, valueDestructor, "function.call");
            LLVMValueRef[] arguments = EmitTransferredArguments(expression.Arguments);
            bool returnsVoid = TypeIdentity.AreSame(expression.Type, BuiltinTypes.Void);
            LLVMValueRef resultAddress = returnsVoid
                ? default
                : _builder.BuildAlloca(_mapType(expression.Type), "function.result");
            LLVMBasicBlockRef direct = _llvmFunction.AppendBasicBlock("function.call.direct");
            LLVMBasicBlockRef closure = _llvmFunction.AppendBasicBlock("function.call.closure");
            LLVMBasicBlockRef end = _llvmFunction.AppendBasicBlock("function.call.end");
            _builder.BuildCondBr(_builder.BuildICmp(LLVMIntPredicate.LLVMIntEQ, control,
                LLVMValueRef.CreateConstPointerNull(control.TypeOf), "function.call.has.no.environment"),
                direct, closure);

            _builder.PositionAtEnd(direct);
            LLVMTypeRef directSignature = LLVMTypeRef.CreateFunction(
                _mapType(expression.FunctionValueType.ReturnType),
                [.. expression.FunctionValueType.ParameterTypes.Select(_mapType)], false);
            LLVMValueRef directResult = EmitPotentiallyThrowingCall(
                directSignature, invoke, arguments, returnsVoid ? string.Empty : "function.direct.result");
            ...
            LLVMTypeRef pointer = LLVMTypeRef.CreatePointer(_context.Int8Type, 0);
            LLVMValueRef environment = _builder.BuildLoad2(pointer,
                FunctionControlField(control, 1, "function.call.environment.address"),
                "function.call.environment");
            LLVMTypeRef closureSignature = LLVMTypeRef.CreateFunction(
                _mapType(expression.FunctionValueType.ReturnType),
                [pointer, .. expression.FunctionValueType.ParameterTypes.Select(_mapType)], false);
            var closureArguments = new LLVMValueRef[arguments.Length + 1];
            closureArguments[0] = environment;
            arguments.CopyTo(closureArguments, 1);
            LLVMValueRef closureResult = EmitPotentiallyThrowingCall(
                closureSignature, invoke, closureArguments, returnsVoid ? string.Empty : "function.closure.result");
            if (!returnsVoid) _builder.BuildStore(closureResult, resultAddress);
            _builder.BuildBr(end);

            _builder.PositionAtEnd(end);
            LLVMValueRef result = returnsVoid
                ? default
                : _builder.BuildLoad2(_mapType(expression.Type), resultAddress, "function.result.value");
            DestroyAndCompleteLifecycleValueGuard(callGuard);
            return result;
        }
        ...
        private LLVMValueRef ScalarCleanupValueAddress(
            LLVMValueRef rootAddress,
            ScalarCleanupEntry cleanup)
        {
            LLVMValueRef address = EmitProjectedAddress(rootAddress, cleanup.Path);
            return cleanup.ValueType is AtomicTypeSymbol atomic
                ? AtomicValueAddress(address, atomic)
                : address;
        }
